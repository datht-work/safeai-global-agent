...
|---|---|
| *(Gõ tự nhiên)* | **Tự động Tuân thủ Quốc gia**: Nhắc đến tên quốc gia (VD: "Việt Nam"), Agent tự động áp dụng bộ luật tương ứng (định tuyến sang spoke `safeai-vietnam-compliance` cho yêu cầu tuân thủ chuyên sâu). |
...
| `/template [ngành] [khu vực]`| **Khung Template Ngành**: Nhả ngay bộ khung PRD chuẩn chỉ cho ngành (VD: `/template fintech eu` hoặc `/template banking vn` hoặc `/template ai vn`). |
...
| `/safeai scan [Ngôn ngữ]` | **Quét Vibe Code**: Rà soát code AI viết để tìm secret keys, thư viện "ảo giác", và lỗi bảo mật. |
| `/safeai verify-prd` | **Đối soát PRD**: Kiểm tra xem code thực tế có làm đúng theo các yêu cầu bảo mật trong PRD không. |
...
**Điều gì xảy ra:** Agent tự động lấy dữ liệu luật pháp sở tại (Luật PDPL 2026, An ninh mạng 2025) và ủy quyền phân tích chuyên sâu cho **Chuyên gia Tuân thủ Việt Nam** (`skills/safeai-vietnam-compliance/SKILL.md`).
...
> `/template fintech eu` hoặc `/template banking vn` hoặc `/template ai vn`
...
> **Ví dụ:**
>
> - `/safeai scan @src/` ➔ Quét toàn bộ thư mục `src`.
> - `/safeai scan @PR` ➔ Quét sự thay đổi code trước khi commit (Git Diff).
> - `/safeai scan python: [Dán code]` ➔ Quét nhanh một đoạn code copy.

**Điều gì xảy ra:** Agent thực hiện kiểm tra chuyên sâu các rủi ro:

- **Thư viện "Ma" (Ghost Dependencies)**: Phát hiện các gói cài đặt không tồn tại hoặc độc hại.
- **Secrets & Keys**: Tìm kiếm API Key hoặc mật khẩu giả định bị bỏ quên trong code.
- **Đối soát Tuân thủ**: Dùng `/safeai verify-prd` để đảm bảo code logic thực sự triển khai đúng các cơ chế bảo mật đã cam kết trong SafeAI PRD.
...
---
<small>Chịu trách nhiệm phát triển bởi nhóm SafeAI-Global Team · Cập nhật Version 6.0.0 · Tháng 07/2026</small>
